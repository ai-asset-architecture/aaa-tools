...
- Target: `aaa-tpl-docs/milestones/*` 與 `aaa-tpl-docs/internal/development/milestones/completion-reports/*`
- Trigger: 驗證通過後自動執行並通知人類

## Inputs
- `milestone_id` (e.g., `v0.5`)
- `approver` (e.g., `@architect`)
- `definition_path` (e.g., `aaa-docs/docs/milestones/v0.5-modular-runtime.md`)
...
2. **Evidence**: 產生 `internal/development/milestones/completion-reports/<id>_completion.md`，包含驗證摘要、`verification_hash`、`definition_hash`。
3. **Definition**: 更新 `definition_path` frontmatter：
   - `status: Completed`
   - `completed_at: <timestamp>`
   - `verification_hash: <hash>`
   - `definition_hash: <hash>`
4. **Index**: 重新執行 `governance.update_index` 於 `milestones` 與 `internal/development/milestones/completion-reports`。
